...
def run(path):
    ...
    File = open(path + 'matlabInput1.txt','a')
    ...
    """
    FileIn = open(path+ 'matlabInput1.txt','rt')
    FileOut = open(path+'matlabInput2.txt','wt')
    for line in FileIn:
    ...
    FileOut = open(path+'matlabInput1.txt','wt')
    for line in FileIn:
        FileOut.write(line.replace('[[','{['))
    FileIn.close()
    FileOut.close()
    
    os.unlink(path+'matlabInput2.txt')
